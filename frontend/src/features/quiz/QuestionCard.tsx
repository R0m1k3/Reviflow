...
export function QuestionCard({
    ...
    return (
        <div className={`bg-white rounded-xl border border-slate-200 shadow-soft-indigo overflow-hidden max-w-2xl mx-auto mt-8 p-8 transition-transform duration-300 ${animation === 'pop' ? 'scale-105' : animation === 'shake' ? 'animate-shake' : ''
            }`}>
            <div className="mb-10">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-xs font-black text-slate-400">{Math.round((currentStep / totalSteps) * 100)}%</span>
                </div>
            ...
            <h2 className="text-3xl font-black text-slate-900 mb-10 leading-tight tracking-tight">{question}</h2>
                ...
                {options.map((option, index) => (
                    <button
                        ...
                        className={`w-full text-left p-5 border-2 rounded-xl transition-all duration-200 group relative ${selectedOption === null
                            ? 'border-slate-100 hover:border-soft-indigo hover:bg-indigo-50/30'
                    ...
                    >
                        <div className="flex items-center">
                            <span className={`flex-shrink-0 h-8 w-8 flex items-center justify-center rounded-full border-2 text-sm font-bold transition-colors ${selectedOption === null
                                ? 'border-slate-200 text-slate-400 group-hover:border-soft-indigo group-hover:text-soft-indigo'
                                : selectedOption === index
                                    ? index === correctAnswer
            ...
            {selectedOption !== null && (
                <div className={`mt-6 p-6 rounded-2xl animate-in fade-in slide-in-from-top-2 duration-300 ${isCorrect ? 'bg-green-50 border border-green-100' : 'bg-orange-50 border border-orange-100'}`}>
                    <div className="flex items-start gap-4">
                        <div className={`text-2xl ${isCorrect ? 'grayscale-0' : 'grayscale-0'}`}>
                            {isCorrect ? '🎉' : '💡'}
                        </div>
                        <div>
                            <h4 className={`font-black uppercase tracking-wider text-xs mb-1 ${isCorrect ? 'text-green-600' : 'text-orange-600'}`}>
